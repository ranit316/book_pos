...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('pnivs', function (Blueprint $table) {
            $table->id();
            $table->foreignId('store_id')->comment('from_store -> here store the store_id of where to come the product')->references('id')->on('stores');
            $table->foreignId('to_store')->comment('to store -> here store login store id')->references('id')->on('stores');
